...
const api = axios.create({
  ...
  timeout: 8000,   // 8s — well within the <2s NFR for local network
})
...
export const authApi = {
  ...
  updateProgress: (resourceId, data) => api.post(`/auth/progress/${resourceId}/`, data),
  toggleBookmark: (resourceId) => api.post(`/auth/bookmark/${resourceId}/`),
  bookmarks: () => api.get('/auth/bookmarks/'),
  videoState:     (resourceId) => api.get(`/auth/video-state/${resourceId}/`),
  activity:       ()           => api.get('/auth/activity/'),
  toggleComplete: (resourceId) => api.post(`/auth/complete/${resourceId}/`),
  isCompleted:    (resourceId) => api.get(`/auth/complete/${resourceId}/`),
}
...
export const feedbackApi = {
  rate: (resourceId, score) => api.post(`/feedback/rate/${resourceId}/`, { score }),
  removeRating: (resourceId) => api.delete(`/feedback/rate/${resourceId}/`),
  comments: (resourceId) => api.get(`/feedback/comments/${resourceId}/`),
  addComment: (resourceId, text) => api.post(`/feedback/comments/${resourceId}/`, { text }),
  updateComment: (commentId, text) => api.patch(`/feedback/comments/edit/${commentId}/`, { text }),
  deleteComment: (commentId) => api.delete(`/feedback/comments/edit/${commentId}/`),
